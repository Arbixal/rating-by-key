...
describe('route-driven character loading', () => {
  ...
  test('retries character loading after a request failure', async () => {
    const characterResponse = {
      name: 'Bixwar',
      class: 'Warrior',
      thumbnail_url: 'https://example.com/bixwar.jpg',
    ...
    const fetchMock = vi.fn()
      .mockRejectedValueOnce(new Error('Temporary character failure'))
    ...
    expect(await screen.findByText('Temporary character failure')).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', {name: 'Retry'}));

    expect(await screen.findByText('Bixwar')).toBeInTheDocument();
...
describe('rating data derivation', () => {
  ...
  test('shows a loading state until static dungeon data loads', async () => {
    const runData: RaiderIORun[] = [
      {
        dungeon: 'Altar of Fangs',
        short_name: 'AOF',
    ...
    expect(screen.getByRole('status')).toHaveTextContent('Loading dungeon data...');
    expect(screen.queryByRole('table')).not.toBeInTheDocument();
  });

  test('retries static dungeon data after a request failure', async () => {
    const runData: RaiderIORun[] = [
      {
        dungeon: 'Altar of Fangs',
        short_name: 'AOF',
    ...
    const fetchMock = vi.fn()
      .mockRejectedValueOnce(new Error('Temporary static data failure'))
      .mockResolvedValueOnce({
        ok: true,
        json: async () => staticData,
      });
    ...
    expect(await screen.findByText('Temporary static data failure')).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', {name: 'Retry'}));
